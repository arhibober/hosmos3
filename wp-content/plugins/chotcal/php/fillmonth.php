	...
	for ( $j = 1 ; $j <= (int)$result[$i]->njours ; $j++ )
		...
		for ( $k = 0 ; $k < (int)$options['nbch'] ; $k++ )
			...
			if ( (int)$result[$i]->nch & $mask )
				...
				if ( $j == 1 )
					{ if ( $j != (int)$result[$i]->njours ) $tab_interv[$k][$jour_reserve] = '1'; }
				else if ( $j == (int)$result[$i]->njours )
					{ if ( $j != 1 ) $tab_interv[$k][$jour_reserve] = '3'; }
				else
					$tab_interv[$k][$jour_reserve] = '2';
				if ( $j == 1 )
	...
	for( $j = 1 ; $j < $dernier_jour ; $j++ )
		...
		if ( $tab_reserv[$k][$j] == false )
			...
			for ( $i = $j+1 ; $i < $dernier_jour+32 ; $i++  )
				{
				if ( $tab_reserv[$k][$i] == false && $tab_vacances[$k][$i] == false )
					$tab_maxstay[$k][$j]++;
				else
					break;
				}
			} 
		else if ( $tab_reserv[$k][$j] == true && $tab_bdid[$k][$j] != -1 )
			{
			// pour le 1er jour d'un booking
			$tab_maxstay[$k][$j] = $tab_njours[$k][$j];
			for ( $i = $j+$tab_njours[$k][$j] ; $i < $dernier_jour+32 ; $i++  )
				{
				if ( $tab_reserv[$k][$i] == false && $tab_vacances[$k][$i] == false )
					$tab_maxstay[$k][$j]++;
				else
